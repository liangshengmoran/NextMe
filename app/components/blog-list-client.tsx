...
export default function BlogListClient({
  ...
  const observerCallback = useCallback(
    (entries) => {
      if (entries[0].isIntersecting && visibleBlogs < restBlogs.length) {
        loadMoreBlogs()
      }
    },
    [visibleBlogs, restBlogs.length, loadMoreBlogs]
  )
  ...
  useEffect(() => {
    ...
    if (
      ...
      loadedImages.length < restBlogs.length
    ) {
      timeoutId = setTimeout(() => {
        setLoadedImages((prevLoadedImages) => [
          ...prevLoadedImages,
          restBlogs[prevLoadedImages.length].slug,
        ])
      }, 100)
    }
          ...
          <div className="flex w-full flex-col">
            {firstBlog.metadata.image && (
              ...
              >
                ...
                <Image
                  className="pointer-events-none absolute right-4 top-[-115px] z-10 w-28"
                  alt={'blogMan'}
                  src={blogMan}
                  priority
            ...
            <p className="text-md mt-2 font-medium tracking-tighter transition-all">
              {firstBlog.metadata.title}
            </p>
            <div className="flex items-center gap-1">
              <p className="text-xs text-neutral-600 dark:text-neutral-400">
                {blogPostDate(firstBlog.metadata.publishedAt)}
              </p>
            </div>
          </div>
        </Link>
